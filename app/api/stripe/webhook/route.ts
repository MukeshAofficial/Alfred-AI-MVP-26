...
export async function POST(req: NextRequest) {
  ...
  if (event.type === 'checkout.session.completed') {
    try {
      ...
      const bookingType = session.metadata?.bookingType || 'spa' // Default to spa booking for backward compatibility
      
      console.log(`Webhook received: ${event.type}`, { serviceId, userId, bookingDate, bookingType })
      console.log('Session metadata:', session.metadata)
      
      if (bookingType === 'restaurant') {
        return await handleRestaurantBooking(supabase, session)
      } else {
        return await handleSpaBooking(supabase, session)
      }
    } catch (error: any) {
...
async function handleSpaBooking(supabase: any, session: Stripe.Checkout.Session) {
  try {
    ...
    console.error('Error processing spa booking:', error)
    return NextResponse.json({ error: `Server error: ${error.message}` }, { status: 500 })
  }
}

async function handleRestaurantBooking(supabase: any, session: Stripe.Checkout.Session) {
  try {
    ...
    const userId = session.metadata?.userId
    const bookingDate = session.metadata?.bookingDate
    const partySize = session.metadata?.partySize
    const tableId = session.metadata?.tableId
    const restaurantName = session.metadata?.restaurantName

    // Check if bookings table exists before trying to insert
    const tableStatus = await restaurantDB.checkTablesExist()
    if (!tableStatus.bookingsExist) {
      console.error('Restaurant bookings table does not exist. Running setup scripts.')
      const setupResult = await restaurantDB.runSetupScripts()
      console.log('Setup result:', setupResult)
      
      if (!setupResult.success) {
        return NextResponse.json({ error: 'Failed to set up database tables for restaurant booking' }, { status: 500 })
      }
    }

    if (!restaurantId) {
      console.error('Missing restaurant ID', { restaurantId, bookingDate, partySize })
      return NextResponse.json({ error: 'Missing required metadata: restaurantId' }, { status: 400 })
    }

    console.log(`Processing successful payment for restaurant: ${restaurantName}`)
    console.log(`User ID: ${userId}, Restaurant ID: ${restaurantId}, Booking Date: ${bookingDate}, Party Size: ${partySize}`)
    ...
    // Format the booking date with time
    const formattedBookingDate = bookingDate 
      ? `${bookingDate}T19:00:00.000Z` // Default to 7:00 PM for restaurant bookings
      : now; // Use current time if booking date is missing

    // Create restaurant booking entry
    try {
      // Check if guest_id exists in profiles table to prevent foreign key errors
      let guestExists = false
      let guestId = userId || 'guest'
      ...
      const bookingData = {
        ...
        guest_id: guestId,
        booking_date: formattedBookingDate,
        party_size: parseInt(partySize as string) || 2,
        status: 'confirmed',
        payment_status: 'paid',
        payment_intent: session.payment_intent as string,
        amount_paid: session.amount_total ? session.amount_total / 100 : 0,
      ...
      console.log('Creating restaurant booking record with data:', bookingData)
      ...
      if (error) {
        console.error('Error creating restaurant booking record:', error)
        console.error('Error code:', error.code)
        console.error('Error details:', error.details)
        
        // Handle foreign key errors similarly to spa bookings
        if (error.code === '23503' && error.details?.includes('guest_id')) {
          console.log('Foreign key error with guest_id, using guest profile as fallback')
          
          // Update booking data to use 'guest' profile
          bookingData.guest_id = 'guest';
          
          // Try again with guest profile
          ...
          if (retryError) {
            console.error('Retry with guest profile also failed:', retryError);
            return NextResponse.json({ error: `Failed to create restaurant booking: ${retryError.message}` }, { status: 500 });
          }
          
          console.log(`Created restaurant booking using guest profile with ID: ${retryBooking?.id}`);
          return NextResponse.json({ received: true, bookingId: retryBooking?.id });
        }
        
        return NextResponse.json({ error: `Failed to create restaurant booking record: ${error.message}` }, { status: 500 })
      }

      console.log(`Restaurant booking created successfully with ID: ${booking?.id}`)
      return NextResponse.json({ received: true, bookingId: booking?.id })
    } catch (error: any) {
      console.error('Error processing restaurant booking:', error)
      return NextResponse.json({ error: `Restaurant booking processing error: ${error.message}` }, { status: 500 })
    }
  } catch (error: any) {
    console.error('Error processing restaurant booking:', error)
    return NextResponse.json({ error: `Server error: ${error.message}` }, { status: 500 })
