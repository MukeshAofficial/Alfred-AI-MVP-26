...
RETURNS TRIGGER AS $$
BEGIN
    -- If the profile doesn't exist yet (in case it was created during signup)
...
create table public.services (
  ...
  constraint services_vendor_id_fkey foreign key (vendor_id) references public.profiles (id) on delete cascade,
  constraint services_category_check check (
    (
      category = any (
        array['restaurant'::text, 'spa'::text, 'tour'::text, 'transport'::text, 'entertainment'::text, 'other'::text]
      )
    )
  ),
...
-- Create an index to improve query performance when filtering by vendor
create index services_vendor_id_idx on public.services (vendor_id);

-- Create an index to improve query performance when filtering by category
...
-- Allow vendors to see only their own services
...
-- Allow guests to view all services
create policy "Guests can view all services"
  on public.services for select
  to authenticated
  using (true);
